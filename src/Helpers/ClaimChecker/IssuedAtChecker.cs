...
{
    class IssuedAtChecker : IClaimChecker
    {
        private const string ClaimName = "iat";

        private int timeToleranceInSeconds;

        public IssuedAtChecker(int timeToleranceInSeconds)
        {
        ...
        public void CheckClaim(JToken token)
        {
            ...
            var issuedAtTime = offset.UtcDateTime;

            if (issuedAtTime.AddSeconds(-1 * timeToleranceInSeconds).CompareTo(DateTime.UtcNow) > 0)
            {
                throw new ValidationException("The token was issued in the future");
